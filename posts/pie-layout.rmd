...
Pausing for a second, it's very easy to identify this is essentially the same dynamic programming solution to the longest increasing sequence problem - a popular algorithum tutorial topic. There is a more efficient ($O(n\log(n))$) [solution](https://en.wikipedia.org/wiki/Longest_increasing_subsequence), but it's a bit over my head. It would be interesting to know if that could be adapted for the labels layout challenge at hand.
...
When it comes to testing, the first thing we'd like to verify: the solution set indeed does not overlap. Recall from ealier, this condition is formularized using $\prec$ relation:
...
Next, we would also like to know our algorithm produces a solution with best possible total weight. This is hard to verify for arbitrary inputs, however, we could do a comparision with a brute force solver for small enough input sizes. The brute force appoach is of course, to check all subsets of input spans. 
